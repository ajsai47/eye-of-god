...
- `broker.ts` — Singleton HTTP daemon on localhost:7899 + SQLite. Auto-launched by the MCP server. Manages peers, channels, agents, shared tasks.
- `server.ts` — MCP stdio server, one per Claude Code instance. Connects to broker, exposes tools, pushes channel notifications.
- `shared/types.ts` — Shared TypeScript types for broker API (peers, agents, channels, tasks).
- `shared/summarize.ts` — Auto-summary generation via gpt-5.4-nano.
- `cli.ts` — CLI utility for inspecting broker state, managing channels and tasks.
- `collab.sh` — Shell helper for subagents to participate in collaboration via bash.
